...
// GET /api/assistant-configs/business/[businessId] - Obtener configuración por business ID
export async function GET(
  ...
  try {
    ...
    try {
      const backendResponse = await fetch(`${API_BASE_URL}/assistant-configs/business/${params.businessId}`, {
        method: 'GET',
        headers: {
      ...
      if (!backendResponse.ok) {
        // Si es 404, devolver null en lugar de error (la config puede no existir aún)
    ...
    console.error('Error en /api/assistant-configs/business/[businessId] GET:', error);
