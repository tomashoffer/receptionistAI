...
// GET /api/businesses - Obtener todos los negocios del usuario
export async function GET(request: NextRequest) {
  try {
    ...
    try {
      const backendResponse = await fetch(`${API_BASE_URL}/businesses`, {
        method: 'GET',
        headers: {
    ...
    console.error('Error en /api/businesses GET:', error);
...
export async function POST(request: NextRequest) {
  try {
    ...
    console.error('Error en /api/businesses POST:', error);
    return new NextResponse('Error interno del servidor', { status: 500 });
  }
}
