...
// POST /api/vapi/business/[businessId]/assistant/spanish - Crear asistente de Vapi en español
export async function POST(
  ...
  try {
    ...
    const timeoutId = setTimeout(() => controller.abort(), 30000); // 30 segundos para crear asistente

    try {
      const backendResponse = await fetch(`${API_BASE_URL}/vapi/business/${params.businessId}/assistant/spanish`, {
        method: 'POST',
        headers: {
    ...
    console.error('Error en /api/vapi/business/[businessId]/assistant/spanish POST:', error);
