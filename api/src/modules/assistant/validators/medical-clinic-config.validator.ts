...
export class MedicalClinicConfigValidator extends BaseIndustryValidator {
  validate(configData: any): { isValid: boolean; errors: string[] } {
    ...
    // Validar campos específicos de clínica médica
    if (configData.configuracionAsistente?.fields) {
      ...
      if (!fieldKeys.includes('establecimiento')) {
        errors.push('Medical clinic requires "establecimiento" field');
      }

      if (!fieldKeys.includes('ubicacion')) {
        errors.push('Medical clinic requires "ubicacion" field');
      }
    }
    ...
    if (configData.informacionEstablecimiento) {
      if (!configData.informacionEstablecimiento.general) {
        errors.push('Medical clinic requires "general" section in informacionEstablecimiento');
      }
    }
